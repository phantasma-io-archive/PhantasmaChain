...
namespace Phantasma.Blockchain.Contracts
{
    public class NativeExecutionContext : ExecutionContext
    {
        ...
        public override ExecutionState Execute(ExecutionFrame frame, Stack<VMObject> stack)
        {
            ...
            {
#if DEBUG
                throw new RuntimeException(frame.VM, $"VM nativecall failed: ABI is missing for contract '{this.Contract.Name}'");
#else                            
            ...
            {
#if DEBUG
                throw new RuntimeException(frame.VM, $"VM nativecall failed: method name not present in the VM stack");
#else
                return ExecutionState.Fault;
#endif
            }
            ...
            {
#if DEBUG
                throw new RuntimeException(frame.VM, $"VM nativecall failed: contract '{this.Contract.Name}' does not have method '{methodName}' in its ABI");
#else
                return ExecutionState.Fault;
#endif
            }
            ...
            {
#if DEBUG
                throw new RuntimeException(frame.VM, $"VM nativecall failed: calling method {methodName} with {stack.Count} arguments instead of {method.parameters.Length}");
#else
                return ExecutionState.Fault;
#endif
            }
            ...
            if (this.Contract.HasInternalMethod(methodName, out gasCost))
            {
                ...
                {
#if DEBUG
                    throw new RuntimeException(frame.VM, $"VM nativecall failed: calling method {methodName} with arguments of wrong type, " + ex.ToString());
#else
            ...
            {
#if DEBUG
                throw new RuntimeException(frame.VM, $"VM nativecall failed: contract '{this.Contract.Name}' is not a valid custom contract");
#else
                return ExecutionState.Fault;
#endif
            }
