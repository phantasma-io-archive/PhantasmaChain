...
namespace Phantasma.Cryptography.Ring
{
    ...
    public class RingSignature : Signature, ILinkableSignature
    {
        ...
        {
            writer.WriteBigInteger(Y0);
            writer.WriteBigInteger(S);
            writer.WriteVarInt(C.Length);
        ...
        public static RingSignature Unserialize(BinaryReader reader)
        {
        ...
        private bool VerifyA(byte[] message, BigInteger[] publicKeys, BigInteger a)
        {
            ...
            var L = ConcatInts(null, publicKeys);
            var h = Hash2(L);
            var prefix = ConcatInts(L, this.Y0).Concat(message);
