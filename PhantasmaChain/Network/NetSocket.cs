...
namespace Phantasma.Network
{
    internal sealed class NetSocket
    {
        ...
        private void ReceiveLogic(object state)
        {
            ...
            var bufferNetEndPoint = new Endpoint((IPEndPoint)bufferEndPoint);
            ...
            while (_running)
            {
                ...
                try
                {
                    ...
                    {
                        bufferNetEndPoint = new Endpoint((IPEndPoint)bufferEndPoint);
                    }
                }
                catch (SocketException ex)
                {
                        ...
                        ex.SocketErrorCode == SocketError.MessageSize)
                    {
                        ...
                        //Just UDP
                        NetUtils.DebugWriteError("[R] Ignored error: {0} - {1}", (int)ex.SocketErrorCode, ex.ToString());
                        continue;
                    }
                ...
                //All ok!
                NetUtils.DebugWrite("[R]Recieved data from {0}, result: {1}", bufferNetEndPoint.ToString(), result);
        ...
        public bool Bind(int port)
        {
            ...
            _localEndPoint = new Endpoint((IPEndPoint)_udpSocketv4.LocalEndPoint);
            ...
            {
                _localEndPoint = new Endpoint((IPEndPoint)_udpSocketv6.LocalEndPoint);
        ...
        private bool BindSocket(Socket socket, IPEndPoint ep)
        {
            ...
            {
                socket.Bind(ep);
                NetUtils.DebugWrite("[B]Succesfully binded to port: {0}", ((IPEndPoint)socket.LocalEndPoint).Port);
            }
            catch (SocketException ex)
            {
        ...
        public int SendTo(byte[] data, int offset, int size, Endpoint remoteEndPoint, ref int errorCode)
        {
            try
            {
                int result = 0;
                ...
                NetUtils.DebugWrite("[S]Send packet to {0}, result: {1}", remoteEndPoint.EndPoint, result);
                return result;
            }
            catch (SocketException ex)
            {
...
namespace LiteNetLib
{
    internal sealed class NetSocket
    {
        ...
        private readonly Dictionary<EndPoint, IOutputStream> _peers = new Dictionary<EndPoint, IOutputStream>();
        private readonly NetManager.OnMessageReceived _onMessageReceived;
        ...
        private void OnMessageReceived(DatagramSocket sender, DatagramSocketMessageReceivedEventArgs args)
        {
            ...
            {
                _bufferEndPoint = new EndPoint(args.RemoteAddress, args.RemotePort);
            }
        ...
        public bool Bind(int port)
        {
            ...
            try
            {
                ...
                _localEndPoint = new EndPoint(_datagramSocket.Information.LocalAddress, _datagramSocket.Information.LocalPort);
            }
            catch (Exception ex)
            {
                NetUtils.DebugWriteError("[B]Bind exception: {0}", ex.ToString());
        ...
        public int SendTo(byte[] data, int offset, int length, EndPoint remoteEndPoint, ref int errorCode)
        {
            Task<uint> task = null;
            try
            {
        ...
        internal void RemovePeer(EndPoint ep)
        {
            _peers.Remove(ep);
        }
