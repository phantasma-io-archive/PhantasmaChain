...
namespace Phantasma.Network
{
    public class NtpSyncModule
    {
        ...
        private readonly Endpoint _ntpEndPoint;
        private readonly ManualResetEvent _waiter = new ManualResetEvent(false);

        public NtpSyncModule(string ntpServer)
        {
            _ntpEndPoint = new Endpoint(ntpServer, 123);
            _socket = new NetSocket(OnMessageReceived);
            _socket.Bind(0);
            SyncedTime = null;
        }

        private void OnMessageReceived(byte[] data, int length, int errorCode, Endpoint remoteEndPoint)
        {
            if (errorCode != 0)
            {
                _waiter.Set();
