...
namespace Phantasma.Network
{
    ...
    {
        internal delegate void OnMessageReceived(byte[] data, int length, int errorCode, Endpoint remoteEndPoint);
        ...
        private readonly Dictionary<Endpoint, NetPeer> _peers;
        private readonly int _maxConnections;
        private readonly Queue<Endpoint> _peersToRemove;
        ...
        public NetManager(INetEventListener listener, int maxConnections, string connectKey)
        {
            ...
            _peers = new Dictionary<Endpoint, NetPeer>();
            _peersToRemove = new Queue<Endpoint>(maxConnections);
        ...
        internal bool SendRaw(byte[] message, Endpoint remoteEndPoint)
        {
            return SendRaw(message, 0, message.Length, remoteEndPoint);
        }

        internal bool SendRaw(byte[] message, int start, int length, Endpoint remoteEndPoint)
        {
            if (!IsRunning)
                return false;
            ...
            if (errorCode == 10040)
            {
                NetUtils.DebugWrite("[SRD] 10040, datalen: {0}", length);
                return false;
            }
#if STATS_ENABLED
            PacketsSent++;
            ...
            int count)
        {
            if (sendDisconnectPacket)
            {
                ...
                var disconnectPacket = Packet.CreateRawPacket(PacketProperty.Disconnect, 8 + count);
                BitUtils.GetBytes(disconnectPacket, 1, peer.ConnectId);
                if (data != null)
                {
        ...
        private void ReceiveLogic(byte[] data, int length, int errorCode, Endpoint remoteEndPoint)
        {
            //Receive some info
            if (errorCode == 0)
            {
        ...
        private void DataReceived(byte[] reusableBuffer, int count, Endpoint remoteEndPoint)
        {
#if STATS_ENABLED
            PacketsReceived++;
            ...
            if (!Packet.GetPacketProperty(reusableBuffer, out property))
                return;
            ...
            switch (property)
            {
                ...
                if(DiscoveryEnabled)
                {
                    ...
                    netEvent.RemoteEndPoint = remoteEndPoint;
                    netEvent.DataReader.SetSource(Packet.GetUnconnectedData(reusableBuffer, count));
                    EnqueueEvent(netEvent);
                }
                return;
                ...
                {
                    if (NatPunchEnabled)
                        NatPunchModule.ProcessMessage(remoteEndPoint, property, Packet.GetUnconnectedData(reusableBuffer, count));
                    return;
                }
            }
            ...
            try
            {
                ...
                if (peersCount < _maxConnections && packet.Property == PacketProperty.ConnectRequest)
                {
                    ...
                    {
                        NetUtils.DebugWrite("[NS] Peer connect reject. Invalid protocol ID: " + protoId);
                        return;
                    }
                    ...
                    {
                        NetUtils.DebugWrite("[NS] Peer connect reject. Invalid key: " + peerKey);
                        return;
                    }
                    ...
                    //response with id
                    NetUtils.DebugWrite("[NS] Received peer connect request Id: {0}, EP: {1}",
        ...
        internal void ReceiveFromPeer(Packet packet, Endpoint remoteEndPoint)
        {
            NetPeer fromPeer;
            if (_peers.TryGetValue(remoteEndPoint, out fromPeer))
            {
                NetUtils.DebugWrite("[NC] Received message");
        ...
        public bool SendUnconnectedMessage(byte[] message, Endpoint remoteEndPoint)
        {
        ...
        /// <param name="remoteEndPoint">Packet destination</param>
        /// <returns>Operation result</returns>
        public bool SendUnconnectedMessage(NetDataWriter writer, Endpoint remoteEndPoint)
        {
        ...
        /// <param name="remoteEndPoint">Packet destination</param>
        /// <returns>Operation result</returns>
        public bool SendUnconnectedMessage(byte[] message, int start, int length, Endpoint remoteEndPoint)
        {
            if (!IsRunning)
                return false;
            var packet = Packet.CreateRawPacket(PacketProperty.UnconnectedMessage, message, start, length);
        ...
        public bool SendDiscoveryRequest(byte[] data, int start, int length, int port)
        {
            if (!IsRunning)
                return false;
            var packet = Packet.CreateRawPacket(PacketProperty.DiscoveryRequest, data, start, length);
            return _socket.SendBroadcast(packet, 0, packet.Length, port);
        }

        public bool SendDiscoveryResponse(NetDataWriter writer, Endpoint remoteEndPoint)
        {
            return SendDiscoveryResponse(writer.Data, 0, writer.Length, remoteEndPoint);
        }

        public bool SendDiscoveryResponse(byte[] data, Endpoint remoteEndPoint)
        {
            return SendDiscoveryResponse(data, 0, data.Length, remoteEndPoint);
        }

        public bool SendDiscoveryResponse(byte[] data, int start, int length, Endpoint remoteEndPoint)
        {
            if (!IsRunning)
                return false;
            var packet = Packet.CreateRawPacket(PacketProperty.DiscoveryResponse, data, start, length);
            return SendRaw(packet, remoteEndPoint);
        }
        ...
        public void Connect(Endpoint target)
        {
            if (!IsRunning)
            {
        ...
        public void Stop()
        {
            ...
            lock (_peers)
                foreach (NetPeer netPeer in _peers.Values)
                {
                    var disconnectPacket = Packet.CreateRawPacket(PacketProperty.Disconnect, 8);
                    BitUtils.GetBytes(disconnectPacket, 1, netPeer.ConnectId);
        ...
        public NetPeer FindPeer(Endpoint endpoint)
        {
            lock (_peers)
            {
