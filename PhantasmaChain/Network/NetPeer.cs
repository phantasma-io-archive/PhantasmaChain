...
namespace Phantasma.Network
{
    ...
    public sealed class NetPeer
    {
        ...
        //Common            
        private readonly Stack<Packet> _packetPool;
        ...
        //Merging
        private readonly Packet _mergeData = new Packet();
        ...
        internal NetPeer(NetManager peerListener, Endpoint remoteEndPoint, long connectId)
        {
            ...
            NetUtils.DebugWrite("[CC] ConnectId: {0}", _connectId);
        }
        ...
        private void SendConnectRequest()
        {
            ...
            //Make initial packet
            var connectPacket = Packet.CreateRawPacket(PacketProperty.ConnectRequest, 12 + keyData.Length);

            //Add data
            BitUtils.GetBytes(connectPacket, 1, NetConstants.ProtocolId);
            BitUtils.GetBytes(connectPacket, 5, _connectId);
        ...
        private void SendConnectAccept()
        {
            ...
            //Make initial packet
            var connectPacket = Packet.CreateRawPacket(PacketProperty.ConnectAccept, 8);

            //Add data
            BitUtils.GetBytes(connectPacket, 1, _connectId);

            //Send raw
            _peerListener.SendRaw(connectPacket, _remoteEndPoint);
        }

        internal bool ProcessConnectAccept(Packet packet)
        {
            ...
            NetUtils.DebugWrite("[NC] Received connection accept");
            _lastPacketReceivedStart = DateTime.UtcNow;
            _connectionState = ConnectionState.Connected;
            return true;
        }
        ...
        {
            return _mtu - Packet.GetHeaderSize(SendOptionsToProperty(options));
        }
        ...
        public void Send(byte[] data, int start, int length, SendOptions options)
        {
            ...
            int headerSize = Packet.GetHeaderSize(property);
            ...
            if (length + headerSize > _mtu)
            {
                ...
                {
                    Packet p = GetPacketFromPool(property, packetFullSize);
                ...
                if (lastPacketSize > 0)
                {
                    Packet p = GetPacketFromPool(property, lastPacketSize + NetConstants.FragmentHeaderSize);
                    p.FragmentId = _fragmentId;
            ...
            //Else just send
            Packet packet = GetPacketFromPool(property, length);
        ...
        {
            Packet packet = GetPacketFromPool(property);
        ...
        {
            NetUtils.DebugWrite(str, args);
        }

        [Conditional("DEBUG_MESSAGES"), Conditional("DEBUG")]
        internal void DebugWriteWarning(string str, params object[] args)
        {
            NetUtils.DebugWriteWarning(str, args);
        }

        internal Packet GetPacketFromPool(PacketProperty property = PacketProperty.Unreliable, int size=0, bool init=true)
        {
            Packet packet = null;
            lock (_packetPool)
            {
        ...
        internal void Recycle(Packet packet)
        {
            packet.RawData = null;
            lock (_packetPool)
            {
        ...
        internal void AddIncomingPacket(Packet p)
        {
            if (p.IsFragmented)
            {
                ...
                if (!_holdedFragments.TryGetValue(packetFragId, out incomingFragments))
                {
                    ...
                    {
                        Fragments = new Packet[p.FragmentsTotal]
                    };
                ...
                Packet resultingPacket = GetPacketFromPool(p.Property, incomingFragments.TotalSize);
        ...
        private void ProcessMtuPacket(Packet packet)
        {
            ...
            switch (packet.Property)
            {
                ...
                case PacketProperty.Merged:
                    ...
                    while (pos < packet.RawData.Length)
                    {
                        ...
                        pos += 2;
                        Packet mergedPacket = GetPacketFromPool(init: false);
                ...
                default:
                    DebugWriteWarning("Error! Unexpected packet type: " + packet.Property);
                    break;
            }
        }
        ...
        internal void SendRawData(byte[] data)
        {
            ...
            {
                BitUtils.GetBytes(_mergeData.RawData, _mergePos + NetConstants.HeaderSize, (ushort)data.Length);
